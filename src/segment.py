...
def fetch_channels(img_id, dir_path, channel_names=config.SEG_CHANNELS):
...
def is_image_edge(img_shape, y_min, y_max, x_min, x_max):
    ...
    if (y_min == 0) or (x_min == 0):
        return True
    if ((y_max + 1) >= img_shape[0]) or ((x_max + 1) >= img_shape[1]):
        return True
    return False
...
def extract_and_save(parent_id, channels, mask, df, destination):
    ...
    for label in np.unique(mask):
        ...
        metadata = {
            ...
            'cell_number': label,
            'size_y': single_cell.shape[0],
            'size_x': single_cell.shape[1],
            'edge_of_img': edge_of_image # is on edge of parent image?
        }
...
def segment_image(img_id, input_dir, output_dir, seg):
    'Segment cells in img_id and save to output_dir then return img metadata'
    channels = fetch_channels(img_id, input_dir)
    ref_channels = channels[:3]
    df = pd.DataFrame()
    mask = get_segmentation_mask(ref_channels, seg)
    df = extract_and_save(img_id, channels, mask, df, output_dir)
    return df
...
def run(input_dir, train_csv, output_dir, nuc_model_path, cell_model_path):
    ...
    for count, img_id, _ in train_df.itertuples():
        df = segment_image(img_id, input_dir, output_dir, seg)
        output_df = output_df.append(df)
        # Checkpoint every 10 images saving progress to csv
        if count % 10 == 0:
