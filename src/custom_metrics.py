'''A set of custom metrics for monitoring network training with extremely sparse class labels'''
...
def skl_mAP(y_preds, y_targets):
    'Return sklearn mean average precision across only labels present'
    ap_rec = []
    # Calc avg precision score one label at a time
    for lab in range(y_targets.shape[1]):
        y_pred = y_preds[:, lab]
        y_target = y_targets[:, lab]
        # If no targets present, skip label to avoid dividing by 0
        if y_target.sum() == 0:
            continue
